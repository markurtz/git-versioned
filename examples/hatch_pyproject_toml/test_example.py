...
__all__ = ["TestHatchPyprojectTomlExample"]


@pytest.mark.regression
class TestHatchPyprojectTomlExample:
    """
    ...
    def test_example_build_and_execution(self, tmp_path: Path) -> None:
        ...
        temp_dir = tmp_path / "hatch_pyproject_toml"
        temp_dir.mkdir()
        ...
        repo.add(".gitignore")
        repo.add("README.md")
        repo.add("src/hatch_pyproject_toml/__init__.py")
        repo.add("src/hatch_pyproject_toml/main.py")
        ...
        with zipfile.ZipFile(wheels[0]) as zip_file:
            ...
            version_file = "hatch_pyproject_toml/version.py"
        ...
        run_result = subprocess.run(
            [sys.executable, "-m", "hatch_pyproject_toml.main"],
            cwd=temp_dir / "src",
            capture_output=True,
            text=True,
            check=False,
