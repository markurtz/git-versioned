...
class TestSetupCfgSettingsSource:
    ...
    def test_invalid_initialization_values(self) -> None:
        ...
        source_cls = typing.cast("typing.Any", SetupCfgSettingsSource)
    ...
    def test_invalid_initialization_missing(self) -> None:
        ...
        with pytest.raises(TypeError):
            SetupCfgSettingsSource(*typing.cast("typing.Any", ()))
    ...
    def test_get_field_value_invalid(
        ...
        instance, _ = valid_instances
        with pytest.raises(TypeError):
            instance.get_field_value(*typing.cast("typing.Any", ()))
    ...
    def test_call_invalid(
        ...
        instance, _ = valid_instances
        with pytest.raises(TypeError):
            typing.cast("typing.Any", instance)(invalid_arg=True)
...
class TestSettings:
    ...
    def test_invalid_initialization_values(self) -> None:
        ...
        data = {"package_name": ["not", "a", "string"]}
        with pytest.raises(ValidationError):
            Settings(**typing.cast("typing.Any", data))

    @pytest.mark.smoke
    def test_initialization_missing_defaults_to_auto(self) -> None:
        """Test Settings with missing initialization arguments defaults to 'auto'."""
        assert Settings.model_fields["package_name"].default == "auto"
    ...
    def test_settings_customise_sources_invalid(self) -> None:
        ...
        with pytest.raises(TypeError):
            Settings.settings_customise_sources(*typing.cast("typing.Any", ()))
    ...
    def test_key_normalization(self) -> None:
        """Test that legacy and dashed keys are correctly normalized."""
        kwargs: dict[str, Any] = {
            "build-backend": "maturin",
            "output_file": "custom.py",
            "template-release": "my-template",
        }
        s = Settings(
            package_name="test",
            **kwargs,
        )
        assert s.build_backend == "maturin"
        assert s.output == "custom.py"
        assert s.pattern_release == "my-template"

    @pytest.mark.smoke
    def test_build_backend_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Test that build_backend is successfully loaded from the environment."""
        monkeypatch.setenv("GITVERSIONED_BUILD_BACKEND", "hatchling.build")
        s = Settings(package_name="test")
        assert s.build_backend == "hatchling.build"
