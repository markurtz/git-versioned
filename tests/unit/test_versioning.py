...
class TestResolveVersion:
    ...
    def test_source_configs(
        ...
        settings = Settings(
            ...
            source_type=["explicit"],  # type: ignore[arg-type]
            version=version_str,
    ...
    def test_source_version_function(
        ...
        settings = Settings(
            package_name="test_pkg",
            source_type=["function"],  # type: ignore[arg-type]
    ...
    def test_source_git_branch(self, branch_name: str, expected: str) -> None:
        ...
        settings = Settings(
            package_name="test_pkg",
            source_type=["branch"],  # type: ignore[arg-type]
            version_type="release",
        )
        version, _ = resolve_version(settings, repository, environment)
        assert str(version) == expected
    ...
    def test_source_git_commit(self, commit_message: str, expected: str) -> None:
        ...
        settings = Settings(
            package_name="test_pkg",
            source_type=["commit"],  # type: ignore[arg-type]
            version_type="release",
        )
        version, _ = resolve_version(settings, repository, environment)
        assert str(version) == expected
    ...
    def test_source_auto(self, tag_name: str, expected: str) -> None:
        ...
        settings = Settings(
            package_name="test_pkg",
            source_type=["auto"],  # type: ignore[arg-type]
            version="auto",
            version_type="release",
        )
        version, _ = resolve_version(settings, repository, environment)
